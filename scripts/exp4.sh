...
# usage: ./exp4.sh [time] [repetitions] [nopop]
...
[ -e exp4_${VM_NAME}.log ] && rm exp4_${VM_NAME}.log

echolog() {
    echo `date +%Y-%m-%d\ %H:%M:%S` $@ | tee -a exp4_${VM_NAME}.log
}
...
if [[ ${type} == "memtier" ]]; then
    ...
    [ -e ${fnamepart} ] && backup="../logs/4/backup_$(date +%Y-%m-%d_%H-%M-%S)_${VM_NAME}.tar.gz" && echolog "Old data folder found, backing up to ${backup}" && tar -czf ${backup} ${fnamepart} && rm -rf ${fnamepart}
    echolog
