...
# usage: ./testrun.sh [time] [repetitions] [nopop]
...
[ -e testrun_${VM_NAME}.log ] && rm testrun_${VM_NAME}.log

echolog() {
    echo `date +%Y-%m-%d\ %H:%M:%S` $@ | tee -a testrun_${VM_NAME}.log
}
...
fnamepart="../logs/testrun"
clients=(32)
workers=(64)

echolog "# ASL section testrun experiments"
...
echolog "# ASL section testrun experiments ALL DONE."